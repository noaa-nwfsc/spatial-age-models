...
d <- bio
d <- dplyr::filter(d, age >= min_age, age <= max_age)
...
saveRDS(fit_svc_age_smooth, paste0("sdmtmb_models/",spp_name,"_model.rds"))
...
dplyr::filter(pred, year == max(d$year)) |>
  ...
  labs(col = "Age effect")
ggsave(paste0("plots/",spp_name,"_spatialage_effect.png"))
...
ggsave(paste0("plots/",spp_name,"_spatialyear_effect.png"))
